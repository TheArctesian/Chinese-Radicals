<div class="container">
	<h1 class="title">Chinese Radical Table</h1>
	<table>
		<thead>
			<tr>
				<th>Pinyin</th>
				<th>Radical</th>
				<th>Variant <br />(Simplified)</th>
				<th>English</th>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td>yī</td>
				<td>一</td>
				<td></td>
				<td>one</td>
			</tr>
			<tr>
				<td>shù</td>
				<td>丨</td>
				<td></td>
				<td>line</td>
			</tr>
			<tr>
				<td>diǎn</td>
				<td>丶</td>
				<td></td>
				<td>dot</td>
			</tr>
			<tr>
				<td>piě</td>
				<td>丿</td>
				<td>乀 乁</td>
				<td>slash</td>
			</tr>
			<tr>
				<td>yǐ</td>
				<td>乙</td>
				<td>乚 乛</td>
				<td>second</td>
			</tr>
			<tr>
				<td>gōu</td>
				<td>亅</td>
				<td></td>
				<td>hook</td>
			</tr>
			<tr>
				<td>èr</td>
				<td>二</td>
				<td></td>
				<td>two</td>
			</tr>
			<tr>
				<td>tóu</td>
				<td>亠</td>
				<td></td>
				<td>lid</td>
			</tr>
			<tr>
				<td>rén</td>
				<td>人</td>
				<td>亻</td>
				<td>person</td>
			</tr>
			<tr>
				<td>ér</td>
				<td>儿</td>
				<td></td>
				<td>legs</td>
			</tr>
			<tr>
				<td>rù</td>
				<td>入</td>
				<td></td>
				<td>enter</td>
			</tr>
			<tr>
				<td>bā</td>
				<td>八</td>
				<td>丷</td>
				<td>eight</td>
			</tr>
			<tr>
				<td>jiǒng</td>
				<td>冂</td>
				<td></td>
				<td>down box</td>
			</tr>
			<tr>
				<td>mì</td>
				<td>冖</td>
				<td></td>
				<td>cover</td>
			</tr>
			<tr>
				<td>bīng</td>
				<td>冫</td>
				<td></td>
				<td>ice</td>
			</tr>
			<tr>
				<td>jī, jǐ</td>
				<td>几</td>
				<td></td>
				<td>table</td>
			</tr>
			<tr>
				<td>qǔ</td>
				<td>凵</td>
				<td></td>
				<td>open box</td>
			</tr>
			<tr>
				<td>dāo</td>
				<td>刀</td>
				<td>刂</td>
				<td>knife</td>
			</tr>
			<tr>
				<td>lì</td>
				<td>力</td>
				<td></td>
				<td>power</td>
			</tr>
			<tr>
				<td>bāo</td>
				<td>勹</td>
				<td></td>
				<td>wrap</td>
			</tr>
			<tr>
				<td>bǐ</td>
				<td>匕</td>
				<td></td>
				<td>ladle</td>
			</tr>
			<tr>
				<td>fāng</td>
				<td>匚</td>
				<td></td>
				<td>right open box</td>
			</tr>
			<tr>
				<td>xǐ</td>
				<td>匸</td>
				<td></td>
				<td>hiding enclosure</td>
			</tr>
			<tr>
				<td>shí</td>
				<td>十</td>
				<td></td>
				<td>ten</td>
			</tr>
			<tr>
				<td>bǔ</td>
				<td>卜</td>
				<td></td>
				<td>divination</td>
			</tr>
			<tr>
				<td>jié</td>
				<td>卩</td>
				<td></td>
				<td>seal</td>
			</tr>
			<tr>
				<td>hàn</td>
				<td>厂</td>
				<td></td>
				<td>cliff</td>
			</tr>
			<tr>
				<td>sī</td>
				<td>厶</td>
				<td></td>
				<td>private</td>
			</tr>
			<tr>
				<td>yòu</td>
				<td>又</td>
				<td></td>
				<td>again</td>
			</tr>
			<tr>
				<td>kǒu</td>
				<td>口</td>
				<td></td>
				<td>mouth</td>
			</tr>
			<tr>
				<td>wéi</td>
				<td>囗</td>
				<td></td>
				<td>enclosure</td>
			</tr>
			<tr>
				<td>tǔ</td>
				<td>土</td>
				<td></td>
				<td>earth</td>
			</tr>
			<tr>
				<td>shì</td>
				<td>士</td>
				<td></td>
				<td>scholar</td>
			</tr>
			<tr>
				<td>zhī</td>
				<td>夂</td>
				<td></td>
				<td>go</td>
			</tr>
			<tr>
				<td>suī</td>
				<td>夊</td>
				<td></td>
				<td>go slowly</td>
			</tr>
			<tr>
				<td>xī</td>
				<td>夕</td>
				<td></td>
				<td>night</td>
			</tr>
			<tr>
				<td>dà</td>
				<td>大</td>
				<td></td>
				<td>big</td>
			</tr>
			<tr>
				<td>nǚ</td>
				<td>女</td>
				<td></td>
				<td>woman</td>
			</tr>
			<tr>
				<td>zǐ</td>
				<td>子</td>
				<td></td>
				<td>child</td>
			</tr>
			<tr>
				<td>mián</td>
				<td>宀</td>
				<td></td>
				<td>roof</td>
			</tr>
			<tr>
				<td>cùn</td>
				<td>寸</td>
				<td></td>
				<td>inch</td>
			</tr>
			<tr>
				<td>xiǎo</td>
				<td>小</td>
				<td></td>
				<td>small</td>
			</tr>
			<tr>
				<td>yóu</td>
				<td>尢</td>
				<td>尣</td>
				<td>lame</td>
			</tr>
			<tr>
				<td>shī</td>
				<td>尸</td>
				<td></td>
				<td>corpse</td>
			</tr>
			<tr>
				<td>chè</td>
				<td>屮</td>
				<td></td>
				<td>sprout</td>
			</tr>
			<tr>
				<td>shān</td>
				<td>山</td>
				<td></td>
				<td>mountain</td>
			</tr>
			<tr>
				<td>chuān</td>
				<td>川</td>
				<td>巛 巜</td>
				<td>river</td>
			</tr>
			<tr>
				<td>gōng</td>
				<td>工</td>
				<td></td>
				<td>work</td>
			</tr>
			<tr>
				<td>jǐ</td>
				<td>己</td>
				<td></td>
				<td>oneself</td>
			</tr>
			<tr>
				<td>jīn</td>
				<td>巾</td>
				<td></td>
				<td>towel</td>
			</tr>
			<tr>
				<td>gān</td>
				<td>干</td>
				<td></td>
				<td>dry</td>
			</tr>
			<tr>
				<td>yāo</td>
				<td>幺</td>
				<td></td>
				<td>thread</td>
			</tr>
			<tr>
				<td>guǎng</td>
				<td>广</td>
				<td></td>
				<td>shelter</td>
			</tr>
			<tr>
				<td>yǐn</td>
				<td>廴</td>
				<td></td>
				<td>stride</td>
			</tr>
			<tr>
				<td>gǒng</td>
				<td>廾</td>
				<td></td>
				<td>hands joined</td>
			</tr>
			<tr>
				<td>yì</td>
				<td>弋</td>
				<td></td>
				<td>shoot with a bow</td>
			</tr>
			<tr>
				<td>gōng</td>
				<td>弓</td>
				<td></td>
				<td>bow</td>
			</tr>
			<tr>
				<td>jì</td>
				<td>彐</td>
				<td>彑</td>
				<td>snout</td>
			</tr>
			<tr>
				<td>shān</td>
				<td>彡</td>
				<td></td>
				<td>hair</td>
			</tr>
			<tr>
				<td>chì</td>
				<td>彳</td>
				<td></td>
				<td>step</td>
			</tr>
			<tr>
				<td>xīn</td>
				<td>心</td>
				<td>忄</td>
				<td>heart</td>
			</tr>
			<tr>
				<td>gē</td>
				<td>戈</td>
				<td></td>
				<td>spear</td>
			</tr>
			<tr>
				<td>hù</td>
				<td>户</td>
				<td></td>
				<td>door</td>
			</tr>
			<tr>
				<td>shǒu</td>
				<td>手</td>
				<td>扌</td>
				<td>hand</td>
			</tr>
			<tr>
				<td>zhī</td>
				<td>支</td>
				<td></td>
				<td>branch</td>
			</tr>
			<tr>
				<td>pū</td>
				<td>攴</td>
				<td>攵</td>
				<td>rap</td>
			</tr>
			<tr>
				<td>wén</td>
				<td>文</td>
				<td></td>
				<td>script</td>
			</tr>
			<tr>
				<td>dǒu</td>
				<td>斗</td>
				<td></td>
				<td>dipper</td>
			</tr>
			<tr>
				<td>jīn</td>
				<td>斤</td>
				<td></td>
				<td>axe</td>
			</tr>
			<tr>
				<td>fāng</td>
				<td>方</td>
				<td></td>
				<td>square</td>
			</tr>
			<tr>
				<td>wú</td>
				<td>无</td>
				<td></td>
				<td>not</td>
			</tr>
			<tr>
				<td>rì</td>
				<td>日</td>
				<td></td>
				<td>sun</td>
			</tr>
			<tr>
				<td>yuē</td>
				<td>曰</td>
				<td></td>
				<td>say</td>
			</tr>
			<tr>
				<td>yuè</td>
				<td>月</td>
				<td></td>
				<td>moon</td>
			</tr>
			<tr>
				<td>mù</td>
				<td>木</td>
				<td></td>
				<td>tree</td>
			</tr>
			<tr>
				<td>qiàn</td>
				<td>欠</td>
				<td></td>
				<td>lack</td>
			</tr>
			<tr>
				<td>zhǐ</td>
				<td>止</td>
				<td></td>
				<td>stop</td>
			</tr>
			<tr>
				<td>dǎi</td>
				<td>歹</td>
				<td></td>
				<td>death</td>
			</tr>
			<tr>
				<td>shū</td>
				<td>殳</td>
				<td></td>
				<td>weapon</td>
			</tr>
			<tr>
				<td>mǔ</td>
				<td>毋</td>
				<td>母</td>
				<td>mother</td>
			</tr>
			<tr>
				<td>bǐ</td>
				<td>比</td>
				<td></td>
				<td>compare</td>
			</tr>
			<tr>
				<td>máo</td>
				<td>毛</td>
				<td></td>
				<td>fur</td>
			</tr>
			<tr>
				<td>shì</td>
				<td>氏</td>
				<td></td>
				<td>clan</td>
			</tr>
			<tr>
				<td>qì</td>
				<td>气</td>
				<td></td>
				<td>steam</td>
			</tr>
			<tr>
				<td>shuǐ</td>
				<td>水</td>
				<td>氵</td>
				<td>water</td>
			</tr>
			<tr>
				<td>huǒ</td>
				<td>火</td>
				<td>灬</td>
				<td>fire</td>
			</tr>
			<tr>
				<td>zhǎo</td>
				<td>爪</td>
				<td>爫</td>
				<td>claw</td>
			</tr>
			<tr>
				<td>fù</td>
				<td>父</td>
				<td></td>
				<td>father</td>
			</tr>
			<tr>
				<td>yáo</td>
				<td>爻</td>
				<td></td>
				<td>lines on a trigram</td>
			</tr>
			<tr>
				<td>qiáng</td>
				<td>爿</td>
				<td></td>
				<td>half of a tree trunk</td>
			</tr>
			<tr>
				<td>piàn</td>
				<td>片</td>
				<td></td>
				<td>slice</td>
			</tr>
			<tr>
				<td>yá</td>
				<td>牙</td>
				<td></td>
				<td>tooth</td>
			</tr>
			<tr>
				<td>niú</td>
				<td>牛</td>
				<td>牜</td>
				<td>cow</td>
			</tr>
			<tr>
				<td>quǎn</td>
				<td>犬</td>
				<td>犭</td>
				<td>dog</td>
			</tr>
			<tr>
				<td>xuán</td>
				<td>玄</td>
				<td></td>
				<td>profound</td>
			</tr>
			<tr>
				<td>yù</td>
				<td>玉</td>
				<td>王</td>
				<td>jade</td>
			</tr>
			<tr>
				<td>guā</td>
				<td>瓜</td>
				<td></td>
				<td>melon</td>
			</tr>
			<tr>
				<td>wǎ</td>
				<td>瓦</td>
				<td></td>
				<td>tile</td>
			</tr>
			<tr>
				<td>gān</td>
				<td>甘</td>
				<td></td>
				<td>sweet</td>
			</tr>
			<tr>
				<td>shēng</td>
				<td>生</td>
				<td></td>
				<td>life</td>
			</tr>
			<tr>
				<td>yòng</td>
				<td>用</td>
				<td></td>
				<td>use</td>
			</tr>
			<tr>
				<td>tián</td>
				<td>田</td>
				<td></td>
				<td>field</td>
			</tr>
			<tr>
				<td>pǐ</td>
				<td>疋</td>
				<td></td>
				<td>cloth</td>
			</tr>
			<tr>
				<td>bìng</td>
				<td>疒</td>
				<td></td>
				<td>ill</td>
			</tr>
			<tr>
				<td>bō</td>
				<td>癶</td>
				<td></td>
				<td>foot steps</td>
			</tr>
			<tr>
				<td>bái</td>
				<td>白</td>
				<td></td>
				<td>white</td>
			</tr>
			<tr>
				<td>pí</td>
				<td>皮</td>
				<td></td>
				<td>skin</td>
			</tr>
			<tr>
				<td>mǐn</td>
				<td>皿</td>
				<td></td>
				<td>dish</td>
			</tr>
			<tr>
				<td>mù</td>
				<td>目</td>
				<td></td>
				<td>eye</td>
			</tr>
			<tr>
				<td>máo</td>
				<td>矛</td>
				<td></td>
				<td>spear</td>
			</tr>
			<tr>
				<td>shǐ</td>
				<td>矢</td>
				<td></td>
				<td>arrow</td>
			</tr>
			<tr>
				<td>shí</td>
				<td>石</td>
				<td></td>
				<td>stone</td>
			</tr>
			<tr>
				<td>shì</td>
				<td>示</td>
				<td>礻</td>
				<td>spirit</td>
			</tr>
			<tr>
				<td>róu</td>
				<td>禸</td>
				<td></td>
				<td>track</td>
			</tr>
			<tr>
				<td>hé</td>
				<td>禾</td>
				<td></td>
				<td>grain</td>
			</tr>
			<tr>
				<td>xuè</td>
				<td>穴</td>
				<td></td>
				<td>cave</td>
			</tr>
			<tr>
				<td>lì</td>
				<td>立</td>
				<td></td>
				<td>stand</td>
			</tr>
			<tr>
				<td>zhú</td>
				<td>竹</td>
				<td></td>
				<td>bamboo</td>
			</tr>
			<tr>
				<td>mǐ</td>
				<td>米</td>
				<td></td>
				<td>rice</td>
			</tr>
			<tr>
				<td>sī</td>
				<td>糸</td>
				<td>(纟)</td>
				<td>silk</td>
			</tr>
			<tr>
				<td>fǒu</td>
				<td>缶</td>
				<td></td>
				<td>jar</td>
			</tr>
			<tr>
				<td>wǎng</td>
				<td>网</td>
				<td>罒</td>
				<td>net</td>
			</tr>
			<tr>
				<td>yáng</td>
				<td>羊</td>
				<td></td>
				<td>sheep</td>
			</tr>
			<tr>
				<td>yǔ</td>
				<td>羽</td>
				<td></td>
				<td>feather</td>
			</tr>
			<tr>
				<td>lǎo</td>
				<td>老</td>
				<td></td>
				<td>old</td>
			</tr>
			<tr>
				<td>ér</td>
				<td>而</td>
				<td></td>
				<td>and</td>
			</tr>
			<tr>
				<td>lěi</td>
				<td>耒</td>
				<td></td>
				<td>plow</td>
			</tr>
			<tr>
				<td>ěr</td>
				<td>耳</td>
				<td></td>
				<td>ear</td>
			</tr>
			<tr>
				<td>yù</td>
				<td>聿</td>
				<td></td>
				<td>brush</td>
			</tr>
			<tr>
				<td>ròu</td>
				<td>肉</td>
				<td></td>
				<td>meat</td>
			</tr>
			<tr>
				<td>chén</td>
				<td>臣</td>
				<td></td>
				<td>minister</td>
			</tr>
			<tr>
				<td>zì</td>
				<td>自</td>
				<td></td>
				<td>oneself</td>
			</tr>
			<tr>
				<td>zhì</td>
				<td>至</td>
				<td></td>
				<td>arrive</td>
			</tr>
			<tr>
				<td>jiù</td>
				<td>臼</td>
				<td></td>
				<td>mortar</td>
			</tr>
			<tr>
				<td>shé</td>
				<td>舌</td>
				<td></td>
				<td>tongue</td>
			</tr>
			<tr>
				<td>chuǎn</td>
				<td>舛</td>
				<td></td>
				<td>contrary</td>
			</tr>
			<tr>
				<td>zhōu</td>
				<td>舟</td>
				<td></td>
				<td>boat</td>
			</tr>
			<tr>
				<td>gèn</td>
				<td>艮</td>
				<td></td>
				<td>mountain</td>
			</tr>
			<tr>
				<td>sè</td>
				<td>色</td>
				<td></td>
				<td>color</td>
			</tr>
			<tr>
				<td>cǎo</td>
				<td>艸</td>
				<td>(艹)</td>
				<td>grass</td>
			</tr>
			<tr>
				<td>hǔ</td>
				<td>虍</td>
				<td></td>
				<td>tiger</td>
			</tr>
			<tr>
				<td>chóng</td>
				<td>虫</td>
				<td></td>
				<td>insect</td>
			</tr>
			<tr>
				<td>xuè</td>
				<td>血</td>
				<td></td>
				<td>blood</td>
			</tr>
			<tr>
				<td>xíng</td>
				<td>行</td>
				<td></td>
				<td>walk</td>
			</tr>
			<tr>
				<td>yī</td>
				<td>衣</td>
				<td>衤</td>
				<td>clothes</td>
			</tr>
			<tr>
				<td>xī</td>
				<td>西</td>
				<td>覀</td>
				<td>west</td>
			</tr>
			<tr>
				<td>jiàn</td>
				<td>見</td>
				<td>(见)</td>
				<td>see</td>
			</tr>
			<tr>
				<td>jiǎo</td>
				<td>角</td>
				<td></td>
				<td>horn</td>
			</tr>
			<tr>
				<td>yán</td>
				<td>言</td>
				<td>(讠)</td>
				<td>speech</td>
			</tr>
			<tr>
				<td>gǔ</td>
				<td>谷</td>
				<td></td>
				<td>valley</td>
			</tr>
			<tr>
				<td>dòu</td>
				<td>豆</td>
				<td></td>
				<td>bean</td>
			</tr>
			<tr>
				<td>shǐ</td>
				<td>豕</td>
				<td></td>
				<td>pig</td>
			</tr>
			<tr>
				<td>zhì</td>
				<td>豸</td>
				<td></td>
				<td>badger</td>
			</tr>
			<tr>
				<td>bèi</td>
				<td>貝</td>
				<td>(贝)</td>
				<td>shell</td>
			</tr>
			<tr>
				<td>chì</td>
				<td>赤</td>
				<td></td>
				<td>red</td>
			</tr>
			<tr>
				<td>zǒu</td>
				<td>走</td>
				<td></td>
				<td>walk</td>
			</tr>
			<tr>
				<td>zú</td>
				<td>足</td>
				<td></td>
				<td>foot</td>
			</tr>
			<tr>
				<td>shēn</td>
				<td>身</td>
				<td></td>
				<td>body</td>
			</tr>
			<tr>
				<td>chē</td>
				<td>車</td>
				<td>(车)</td>
				<td>cart</td>
			</tr>
			<tr>
				<td>xīn</td>
				<td>辛</td>
				<td></td>
				<td>bitter</td>
			</tr>
			<tr>
				<td>chén</td>
				<td>辰</td>
				<td></td>
				<td>morning</td>
			</tr>
			<tr>
				<td>chuò</td>
				<td>辶</td>
				<td></td>
				<td>walk</td>
			</tr>
			<tr>
				<td>yì</td>
				<td>邑</td>
				<td>阝</td>
				<td>city</td>
			</tr>
			<tr>
				<td>yǒu</td>
				<td>酉</td>
				<td></td>
				<td>wine</td>
			</tr>
			<tr>
				<td>biàn</td>
				<td>釆</td>
				<td></td>
				<td>distinguish</td>
			</tr>
			<tr>
				<td>lǐ</td>
				<td>里</td>
				<td></td>
				<td>village</td>
			</tr>
			<tr>
				<td>jīn</td>
				<td>金</td>
				<td>(钅)</td>
				<td>metal</td>
			</tr>
			<tr>
				<td>cháng</td>
				<td>長</td>
				<td>(长)</td>
				<td>long</td>
			</tr>
			<tr>
				<td>mén</td>
				<td>門</td>
				<td>(门)</td>
				<td>gate</td>
			</tr>
			<tr>
				<td>fù</td>
				<td>阜</td>
				<td>阝</td>
				<td>mound</td>
			</tr>
			<tr>
				<td>lì</td>
				<td>隶</td>
				<td></td>
				<td>slave</td>
			</tr>
			<tr>
				<td>zhuī</td>
				<td>隹</td>
				<td></td>
				<td>short-tailed bird</td>
			</tr>
			<tr>
				<td>yǔ</td>
				<td>雨</td>
				<td></td>
				<td>rain</td>
			</tr>
			<tr>
				<td>qīng</td>
				<td>青</td>
				<td></td>
				<td>blue</td>
			</tr>
			<tr>
				<td>fēi</td>
				<td>非</td>
				<td></td>
				<td>wrong</td>
			</tr>
			<tr>
				<td>miàn</td>
				<td>面</td>
				<td></td>
				<td>face</td>
			</tr>
			<tr>
				<td>gé</td>
				<td>革</td>
				<td></td>
				<td>leather</td>
			</tr>
			<tr>
				<td>wěi</td>
				<td>韋</td>
				<td>(韦)</td>
				<td>soft leather</td>
			</tr>
			<tr>
				<td>jiǔ</td>
				<td>韭</td>
				<td></td>
				<td>leek</td>
			</tr>
			<tr>
				<td>yīn</td>
				<td>音</td>
				<td></td>
				<td>sound</td>
			</tr>
			<tr>
				<td>yè</td>
				<td>頁</td>
				<td>(页)</td>
				<td>page</td>
			</tr>
			<tr>
				<td>fēng</td>
				<td>風</td>
				<td>(风)</td>
				<td>wind</td>
			</tr>
			<tr>
				<td>fēi</td>
				<td>飛</td>
				<td>(飞)</td>
				<td>fly</td>
			</tr>
			<tr>
				<td>shí</td>
				<td>食</td>
				<td>飠 (饣)</td>
				<td>eat</td>
			</tr>
			<tr>
				<td>shǒu</td>
				<td>首</td>
				<td></td>
				<td>head</td>
			</tr>
			<tr>
				<td>xiāng</td>
				<td>香</td>
				<td></td>
				<td>fragrant</td>
			</tr>
			<tr>
				<td>mǎ</td>
				<td>馬</td>
				<td>(马)</td>
				<td>horse</td>
			</tr>
			<tr>
				<td>gǔ</td>
				<td>骨</td>
				<td></td>
				<td>bone</td>
			</tr>
			<tr>
				<td>gāo</td>
				<td>高</td>
				<td></td>
				<td>high</td>
			</tr>
			<tr>
				<td>biāo</td>
				<td>髟</td>
				<td></td>
				<td>long hair</td>
			</tr>
			<tr>
				<td>dòu</td>
				<td>鬥</td>
				<td>(斗)</td>
				<td>fight</td>
			</tr>
			<tr>
				<td>chàng</td>
				<td>鬯</td>
				<td></td>
				<td>sacrificial wine</td>
			</tr>
			<tr>
				<td>lì</td>
				<td>鬲</td>
				<td></td>
				<td>cauldron</td>
			</tr>
			<tr>
				<td>guǐ</td>
				<td>鬼</td>
				<td></td>
				<td>ghost</td>
			</tr>
			<tr>
				<td>yú</td>
				<td>魚</td>
				<td>(鱼)</td>
				<td>fish</td>
			</tr>
			<tr>
				<td>niǎo</td>
				<td>鳥</td>
				<td>(鸟)</td>
				<td>bird</td>
			</tr>
			<tr>
				<td>lǔ</td>
				<td>鹵</td>
				<td>(卤)</td>
				<td>salty</td>
			</tr>
			<tr>
				<td>lù</td>
				<td>鹿</td>
				<td></td>
				<td>deer</td>
			</tr>
			<tr>
				<td>mài</td>
				<td>麥</td>
				<td>(麦)</td>
				<td>wheat</td>
			</tr>
			<tr>
				<td>má</td>
				<td>麻</td>
				<td></td>
				<td>hemp</td>
			</tr>
			<tr>
				<td>huáng</td>
				<td>黄</td>
				<td></td>
				<td>yellow</td>
			</tr>
			<tr>
				<td>shǔ</td>
				<td>黍</td>
				<td></td>
				<td>millet</td>
			</tr>
			<tr>
				<td>hēi</td>
				<td>黑</td>
				<td></td>
				<td>black</td>
			</tr>
			<tr>
				<td>zhǐ</td>
				<td>黹</td>
				<td></td>
				<td>embroidery</td>
			</tr>
			<tr>
				<td>mǐn</td>
				<td>黽</td>
				<td>(黾)</td>
				<td>frog</td>
			</tr>
			<tr>
				<td>dǐng</td>
				<td>鼎</td>
				<td></td>
				<td>tripod</td>
			</tr>
			<tr>
				<td>gǔ</td>
				<td>鼓</td>
				<td></td>
				<td>drum</td>
			</tr>
			<tr>
				<td>shǔ</td>
				<td>鼠</td>
				<td></td>
				<td>rat</td>
			</tr>
			<tr>
				<td>bí</td>
				<td>鼻</td>
				<td></td>
				<td>nose</td>
			</tr>
			<tr>
				<td>qí</td>
				<td>齊</td>
				<td>(齐)</td>
				<td>even</td>
			</tr>
			<tr>
				<td>chǐ</td>
				<td>齒</td>
				<td>(齿)</td>
				<td>tooth</td>
			</tr>
			<tr>
				<td>lóng</td>
				<td>龍</td>
				<td>(龙)</td>
				<td>dragon</td>
			</tr>
			<tr>
				<td>guī</td>
				<td>龜</td>
				<td>(龟)</td>
				<td>turtle</td>
			</tr>
			<tr>
				<td>yuè</td>
				<td>龠</td>
				<td></td>
				<td>flute</td>
			</tr>
		</tbody>
	</table>
</div>

<style>
	@import url('https://fonts.googleapis.com/css2?family=Noto+Serif+SC:wght@400;700&family=Noto+Serif:ital,wght@0,400;0,700;1,400&display=swap');

	body {
		font-family: 'Noto Serif', serif;
		background-color: #f9f6f2;
		color: #3a3a3a;
		line-height: 1.6;
		padding: 20px;
		max-width: 1200px;
		margin: 0 auto;
	}

	.container {
		background-color: #fff;
		border-radius: 8px;
		padding: 30px;
		box-shadow: 0 4px 15px rgba(0, 0, 0, 0.05);
		position: relative;
		background-image:
			linear-gradient(rgba(255, 255, 255, 0.7) 1px, transparent 1px),
			linear-gradient(90deg, rgba(255, 255, 255, 0.7) 1px, transparent 1px),
			linear-gradient(#f5f0e8 1px, transparent 1px),
			linear-gradient(90deg, #f5f0e8 1px, transparent 1px);
		background-size:
			100px 100px,
			100px 100px,
			20px 20px,
			20px 20px;
		border: 1px solid #e8e0d2;
	}

	.title {
		font-family: 'Noto Serif', serif;
		font-size: 2rem;
		color: #5a4835;
		text-align: center;
		margin-bottom: 2rem;
		border-bottom: 2px solid #e0d5c5;
		padding-bottom: 1rem;
		letter-spacing: 1px;
		text-shadow: 1px 1px 0 rgba(255, 255, 255, 0.8);
	}

	table {
		width: 100%;
		border-collapse: collapse;
		font-size: 1rem;
		border: 1px solid #e0d5c5;
		margin-top: 1rem;
		background-color: rgba(255, 255, 255, 0.7);
	}

	thead {
		background-color: #f0e8dd;
	}

	th {
		padding: 15px 10px;
		border-bottom: 2px solid #d9cebb;
		color: #5a4835;
		font-weight: bold;
		text-align: left;
	}

	td {
		padding: 12px 10px;
		border-bottom: 1px solid #e8e0d2;
		vertical-align: middle;
		transition: background-color 0.3s;
	}

	tr:nth-child(even) {
		background-color: rgba(245, 240, 232, 0.5);
	}

	tr:hover {
		background-color: rgba(228, 219, 204, 0.5);
	}

	/* Column styles */
	td:nth-child(1) {
		color: #7b6b56;
		font-style: italic;
		width: 15%;
	}

	td:nth-child(2) {
		font-family: 'Noto Serif SC', serif;
		font-size: 1.5rem;
		color: #5a4835;
		text-align: center;
		width: 15%;
	}

	td:nth-child(3) {
		font-family: 'Noto Serif SC', serif;
		font-size: 1.3rem;
		color: #8c7c68;
		text-align: center;
		width: 20%;
	}

	td:nth-child(4) {
		color: #5a4835;
		width: 50%;
	}

	/* Ink brush effect for the radical column */
	td:nth-child(2) {
		position: relative;
	}

	td:nth-child(2)::after {
		content: '';
		position: absolute;
		bottom: -1px;
		left: 25%;
		right: 25%;
		height: 2px;
		background-color: rgba(90, 72, 53, 0.2);
		border-radius: 50%;
	}

	/* Responsive styles */
	@media (max-width: 768px) {
		body {
			padding: 10px;
		}

		.container {
			padding: 15px;
		}

		.title {
			font-size: 1.5rem;
		}

		table {
			font-size: 0.9rem;
		}

		th,
		td {
			padding: 8px 5px;
		}

		td:nth-child(2) {
			font-size: 1.3rem;
		}

		td:nth-child(3) {
			font-size: 1.1rem;
		}
	}

	/* Enhance mobile experience */
	@media (max-width: 500px) {
		th:nth-child(3),
		td:nth-child(3) {
			display: none; /* Hide the variant column on very small screens */
		}

		td:nth-child(1) {
			width: 20%;
		}

		td:nth-child(2) {
			width: 20%;
		}

		td:nth-child(4) {
			width: 60%;
		}
	}
</style>
